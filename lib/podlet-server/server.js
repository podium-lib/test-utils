...
class PodletServer extends EventEmitter {
    constructor({
        ...
        this._app = http.createServer(async (req, res) => {
            ...
            if (inc.url.pathname.startsWith('/public') && req.method === 'GET') {
                res.statusCode = 200;
                res.setHeader('Content-Type', 'application/json');
                res.setHeader('podlet-version', this._podlet.version);
            ...
            if (inc.url.pathname.startsWith('/public') && req.method === 'POST') {
                const payload = [];
                req.on('data', (chunk) => {
                    payload.push(chunk);
                }).on('end', () => {
            ...
            if (inc.url.pathname.startsWith('/public') && req.method === 'PUT') {
                const payload = [];
                req.on('data', (chunk) => {
                    payload.push(chunk);
                }).on('end', () => {
